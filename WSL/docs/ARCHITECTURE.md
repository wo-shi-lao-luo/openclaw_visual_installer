...
Windows users need a reliable way to install OpenClaw without deep terminal knowledge. This document covers the legacy WSL product area, where the installer must make the WSL path understandable, safe, recoverable, and supportable.
...
- This architecture applies to the legacy WSL-first installer area only; native Windows runtime work belongs in `../Windows/`
- MVP in this area is WSL-first only
...
- The installer must stay separate from the future full management GUI and from the native Windows product area
...
- Windows-native runtime mode, which is defined in `../Windows/`
...
Native Windows install/runtime orchestration is out of scope for this area and belongs in `../Windows/`.
...
These phases describe the legacy WSL area only. Native Windows phases will be defined with the `../Windows/` product area.
