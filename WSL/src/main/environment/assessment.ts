 ...
 *
 * In native mode, WSL blockers are skipped. Readiness is determined by Windows version
...
export function buildAssessment(
  ...
  if (mode === 'native' && !npmAvailable) {
    blockers.push({
      code: 'NPM_NOT_AVAILABLE',
      message: 'npm was not found on this system.',
      canAutoResolve: false,
      userAction: 'Install Node.js and ensure npm is on your PATH before using native installation.'
    })
  }
...
function detectNpmAvailability(): boolean {
  const command = process.platform === 'win32' ? 'where npm' : 'command -v npm'
...
export async function runEnvironmentAssessment(mode: InstallMode = 'wsl'): Promise<EnvironmentAssessment> {
  ...
  const npmAvailable = mode === 'native' ? detectNpmAvailability() : true
  return buildAssessment(windowsVersion, wslStatus, mode, npmAvailable)
}
